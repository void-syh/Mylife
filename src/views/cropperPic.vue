<template>
  <div>
    ...
    <el-card id="pic-input-body">
      <h3>上传图片</h3>
      <form id="uploadForm" enctype="multipart/form-data">
            <input id="file" type="file" name="file" />
            <input v-model="phone" type="text" name="phone" id="phone" style="display:none">
      </form>
      <el-button @click="inputPic" type="primary" id="input-button">提交照片</el-button>
    </el-card>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      inputPic(){
      var f = new FormData($('#uploadForm')[0]); //将文件进行格式转化，传至后端
      $.ajax({
      type: 'post',
      url: "http://47.102.143.184:8080/MyLife/image/upload.do",
      data: f,
      cache: false,
      processData: false,
      contentType: false,
      ...
      finish2() {
        this.$refs.cropper2.getCropData((data) => {
          ...
          let blob = this.toBlob(this.example2.img);
          let evt = document.createEvent("HTMLEvents");
          evt.initEvent("click", true, true); //initEvent 不加后两个参数在FF下会报错  事件类型，是否冒泡，是否阻止浏览器的默认行为
          let aLink = document.createElement('a');
          aLink.download = 'cropimg';
